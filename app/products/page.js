...
const Products = () => {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      // Fetch multiple times to get more products for demo
      const promises = [];
      for (let i = 1; i <= 5; i++) {
      ...
      // Add unique IDs to avoid duplicates in React keys
      ...
      {/* Filters */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            {/* Products per page */}
            <div>
              <label htmlFor="perPage" className="block text-sm font-semibold text-gray-700 mb-3">
                📄 Items per page
              </label>
              <select
                id="perPage"
              ...
              <div className="mt-8 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="text-sm text-gray-700">
                    Showing <span className="font-semibold">{startIndex + 1}</span> to <span className="font-semibold">{Math.min(endIndex, filteredProducts.length)}</span> of <span className="font-semibold">{filteredProducts.length}</span> results
                  </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            <div className="text-gray-400 mb-6">
              <svg className="w-20 h-20 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2 2v-5m16 0h-2M4 13h2m13-8V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v1M9 4V3a1 1 0 011-1h4a1 1 0 011 1v1" />
              </svg>
            </div>
